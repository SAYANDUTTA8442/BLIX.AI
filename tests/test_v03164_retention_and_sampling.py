...
class TestRewardLogAutoprune:
    ...
    def test_reward_stats_default_limit_is_1000(self, store):
        """
        reward_stats() default last_n resolves to 1000 from settings.
        The signature default is None (sentinel); the actual value
        comes from adma_settings.reward_log.stats_last_n.
        """
        import inspect
        sig = inspect.signature(store.reward_stats)
        # Signature default is None (settings-resolved) — verify the resolved value
        from config.settings import adma_settings
        assert adma_settings.reward_log.stats_last_n == 1000
