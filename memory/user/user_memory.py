...
# ── user_id validation (ISSUE-010) ───────────────────────────────────
# user_id is inserted into SQLite tags as the string f"user:{user_id}".
# An unvalidated user_id could:
#   - corrupt tag parsing if it contains delimiter characters
#   - enable prompt injection if inserted into compiled prompts
#   - create unbounded cache keys in MemoryManager._users dict
#
# Allowlist: alphanumeric, underscore, hyphen, dot; 1–128 chars.
# This covers email-style IDs (user@domain → use user_domain),
# UUIDs, numeric IDs, and short handles.
_USER_ID_PATTERN = re.compile(r'^[a-zA-Z0-9_\-\.]{1,128}$')


def validate_user_id(user_id: str) -> str:
    """
    Validate and return a safe user_id.

    Raises ValueError with a descriptive message if invalid.
    The allowlist is intentionally strict: only alphanumeric characters,
    hyphens, underscores, and dots are permitted.  This prevents:
      - Tag corruption (no quotes, angle brackets, or JSON special chars)
      - SQL injection (parameterised queries already prevent this, but
        defence-in-depth is worthwhile)
      - Prompt injection via user_id embedded in compiled prompts

    Parameters
    ----------
    user_id : str
        The candidate user identifier.

    Returns
    -------
    str
        The same string, unchanged, if valid.

    Raises
    ------
    ValueError
        If user_id is empty, too long, or contains disallowed characters.
    """
    if not isinstance(user_id, str):
        raise ValueError(
            f"user_id must be a string, got {type(user_id).__name__!r}."
        )
    if not user_id:
    ...
    if not _USER_ID_PATTERN.match(user_id):
        raise ValueError(
            ...
            f"Only alphanumeric characters, hyphens, underscores, and dots "
            f"are allowed (1–128 chars). "
            f"Example valid IDs: 'alice', 'user-42', 'user_001', 'a1b2c3d4'."
        )
    return user_id
...
class UserMemory:
    ...
    def __init__(self, hgshm: HGSHM, user_id: str) -> None:
        self._hgshm = hgshm
        self.user_id = validate_user_id(user_id)   # ISSUE-010: validate on construction
        self._user_tag = f"user:{self.user_id}"
        log.debug("UserMemory initialised for user=%s", self.user_id)

    # ── Context manager (ISSUE-011) ───────────────────────────────────

    def __enter__(self) -> "UserMemory":
        return self

    def __exit__(self, *_) -> None:
        pass  # HGSHM connection is managed by the HGSHM instance
