...
class HGSHMStore:
    ...
    def __init__(self, memory_dir: Path, db_filename: str | None = None) -> None:
        ...
        if db_filename is None:
            try:
                ...
                db_filename = hgshm_settings.database.hgshm_db
            except ImportError:
                db_filename = "hgshm.db"
    ...
    def all_nodes(
        ...
        A05 fix: max_confidence and max_importance push upper-bound
        prune filters to SQL, avoiding full table scan in Python.
        """
        ...
        if max_importance is not None:
            clauses.append("importance <= ?"); params.append(max_importance)
        if memory_type:
