...
class VectorStore:
    ...
    def __init__(self, memory_dir: Path, dim: int = 256, db_filename: str | None = None) -> None:
        memory_dir.mkdir(parents=True, exist_ok=True)
        # A06: configurable filename
        if db_filename is None:
            try:
                from config.settings import hgshm_settings
                db_filename = hgshm_settings.database.vectors_db
            except ImportError:
