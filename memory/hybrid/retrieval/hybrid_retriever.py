...
def _default_retrieval_weights():
    try:
        ...
        # A12: real config error must not be silently swallowed
        import logging as _l
        _l.getLogger(__name__).error(
            "Failed to load retrieval weights from settings: %s — using defaults", exc)
        return {}
    ...
    def recent(self, top_k: int = 20, max_age_hours: float = 24.0) -> list[RetrievedMemory]:
        ...
        for node in nodes:
            try:
                ...
                # A12: node has malformed created_dt or score data — skip and log
                log.warning("TemporalRetriever: skipping node with bad data (%s): %s",
                            getattr(node, 'node_id', '?'), exc)
                continue
            except Exception:
                # A12: unexpected error — log at ERROR but keep retrieval alive
                log.error("TemporalRetriever: unexpected error on node %s",
        ...
        for rm in merged.values():
            ...
            if context_vec is not None:
                try:
                    ...
                    # A12: embedding failure for this node — ctx_score stays 0.0
                    log.warning("HybridRetriever: embed failed for node %s: %s",
                                getattr(node, 'node_id', '?'), exc)
                except Exception:
                    # A12: unexpected error — log with traceback, ctx_score stays 0.0
                    log.error("HybridRetriever: unexpected embed error for node %s",
                              getattr(node, 'node_id', '?'), exc_info=True)
