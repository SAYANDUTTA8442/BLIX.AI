...
# ── Prompt injection mitigation (ISSUE-010) ───────────────────────────
# user_id and task text are inserted into compiled prompts.
# We apply two defences:
#   1. user_id: validated by UserMemory.validate_user_id before reaching
#      the compiler (upstream defence).  The compiler also re-validates
#      for defence-in-depth.
#   2. task text: stripped of known prompt-injection patterns before
#      being placed in task_instructions.
#
# The task stripping is conservative — it removes sequences that look
# like instruction overrides ("ignore previous instructions", role
...
def sanitize_task_text(text: str, max_length: int = 4000) -> str:
    """
    Remove known prompt-injection patterns from user-supplied task text.

    This is a best-effort defence, not a complete solution.  The primary
    defence is to keep user-supplied content in ``task_instructions``
    (not in ``system_instructions``) so the LLM's instruction hierarchy
    applies naturally.
    ...
    text : str
        Raw user task text.
    max_length : int
        Maximum length to pass through (longer text is truncated).

    Returns
    -------
    str
        Sanitized text, safe to include in a compiled prompt.
    """
    # Truncate first to limit regex work
...
def _default_compiler_cfg():
    ...
    if _adma_settings is None:
        return None
    return _adma_settings.prompt_compiler
...
class PolicyCompiler:
    ...
    def compile(
        ...
        context: dict[str, Any] | None = None,
        max_memory_nodes: int | None = None,
