...
try:
    from config.settings import adma_settings as _adma_settings
except ImportError:
    _adma_settings = None  # config module absent — hardcoded defaults used
...
# Settings are imported lazily to avoid circular imports and to allow
# tests to override them before PolicyLearner is instantiated.
def _default_learner_cfg():
    """Return learner config from the module-level singleton (A19: fail-fast import)."""
    if _adma_settings is None:
        return None
    return _adma_settings.learner
...
class PolicyLearner:
    ...
    def __init__(
        ...
        decay_persist_every: int | None = None,
        cache_max_size: int | None = None,
    ) -> None:
        # Read defaults from central settings (ISSUE-014).
        # Explicit constructor arguments take precedence over settings.
        _cfg = _default_learner_cfg()
        self._store           = policy_store
        self._decay_factor    = decay_factor       if decay_factor       is not None else (_cfg.decay_factor       if _cfg else 0.995)
        self._threshold       = reward_threshold   if reward_threshold   is not None else (_cfg.reward_threshold   if _cfg else 0.5)
        self._snapshot_every  = snapshot_every     if snapshot_every     is not None else (_cfg.snapshot_every     if _cfg else 20)
        self._decay_persist_every = decay_persist_every if decay_persist_every is not None else (_cfg.decay_persist_every if _cfg else 50)
        _max = cache_max_size if cache_max_size is not None else (_cfg.cache_max_size if _cfg else 1000)

        # Bounded LRU cache (ISSUE-004)
        self._cache_max_size = _max
        self._cache: OrderedDict[str, PolicyRecord] = OrderedDict()
        # A14: protect LRU cache against concurrent access from multi-threaded callers
        ...
        # ── Per-policy epoch tracking (A01 fix) ──────────────────
        # Track the epoch value at the time each policy was last
        # written to DB.  When a policy is loaded from DB after
        # eviction, we apply only the DELTA epoch (current / at_write)
        # not the full epoch — preventing double-decay.
        self._epoch_at_last_write: dict[str, float] = {}
    ...
    def observe(self, reward: RewardSignal) -> list[PolicyRecord]:
        ...
        if reward.policy_id:
            ...
            ctx_key = _context_key(reward.context) if reward.context else "global"
            for pt in inferred_types:
                policy_id = self._last_selected.get(ctx_key)
                if policy_id is None:
                    log.debug(
                        "observe: broadcast reward dropped — no recent "
                        "selection for ctx=%s type=%s", ctx_key, pt.value
                    )
                    continue
                ...
                if arm and arm.is_active and arm.policy_type == pt:
                    ...
                    log.debug(
                        ...
                        "active or wrong type for ctx=%s", policy_id, ctx_key
                    )
    ...
    def _apply_delta_epoch(self, policy: "PolicyRecord") -> None:
        """
        Apply only the epoch delta since this policy was last written to DB.

        When a policy is loaded from DB after LRU eviction, the DB value
        is the state at the last flush.  The in-process epoch has advanced
        since then.  We apply only the ratio (current_epoch / epoch_at_write)
        so that arms are decayed exactly once per observation, not twice.

        If no write epoch is recorded for this policy (first load ever),
        we apply the full current epoch — this is correct because no
        in-memory decay has been applied to this arm yet.
        """
        epoch_at_write = self._epoch_at_last_write.get(policy.policy_id, 1.0)
        if epoch_at_write <= 0:
            epoch_at_write = 1.0
        delta = self._decay_epoch / epoch_at_write
        if abs(delta - 1.0) < 1e-10:
            return
        policy.alpha = 1.0 + (policy.alpha - 1.0) * delta
        policy.beta_ = 1.0 + (policy.beta_ - 1.0) * delta
    ...
    def flush_decay(self, exclude_ids: set[str] | None = None) -> int:
        ...
        for arm in all_arms:
            ...
            # A01: update the write-epoch for this arm
            self._epoch_at_last_write[arm.policy_id] = 1.0  # epoch resets after flush
            written += 1
    ...
    def _cache_put(self, policy_id: str, policy: "PolicyRecord") -> None:
        ...
        Thread-safe: protected by _cache_lock (A14).
        """
    ...
    def _get_cached(self, policy_id: str) -> PolicyRecord | None:
        ...
        if p:
            # A01 fix: apply only the epoch delta since this policy was
            # last written to DB, not the full accumulated epoch.
            # Using the full epoch would double-decay any arm that was
            # already decayed in memory before being evicted from cache.
            self._apply_delta_epoch(p)
    ...
    def learning_curve(self, policy_id: str) -> list[dict[str, Any]]:
        ...
        return [{"version": v.version, "confidence": v.mean_reward,
                 "alpha": v.alpha, "beta_": v.beta_} for v in history]
