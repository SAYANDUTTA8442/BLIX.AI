...
try:
    from config.settings import hgshm_settings as _hgshm_settings
except ImportError:
    _hgshm_settings = None  # config module absent — hardcoded defaults used
...
class PolicyStore:
    ...
    def __init__(self, memory_dir: Path, db_filename: str | None = None) -> None:
        memory_dir.mkdir(parents=True, exist_ok=True)
        # A06: configurable filename — reads from settings, caller can override
        if db_filename is None:
            if _hgshm_settings is not None:
                db_filename = _hgshm_settings.database.policy_db
            else:
                db_filename = "policy.db"
        self._db_path = memory_dir / db_filename
    ...
    def save_version(self, version: PolicyVersion) -> None:
        ...
        d = version.to_dict()
        params = {**d, "config_json": json.dumps(d["config"])}
        with self._lock:
            try:
                with self._conn:
                    self._conn.execute("""
    ...
    def log_reward(self, reward: RewardSignal, max_rows_per_policy: int | None = None) -> None:
        """
        ...
        """
        # Resolve max_rows from settings when not explicitly passed (ISSUE-014)
        if max_rows_per_policy is None:
        ...
        with self._lock:
            ...
            if max_rows_per_policy > 0 and pid is not None:
                # A04: initialise counter from DB on first write after restart
                # so the limit is respected even if rows already exist in DB.
                # We subtract 1 because reward_log_count() is called after the
                # INSERT above — the new row is already counted, so the
                # pre-insert count is (db_count - 1), and then += 1 yields db_count.
                if pid not in self._reward_log_counts:
                    self._reward_log_counts[pid] = self.reward_log_count(pid) - 1
    ...
    def reward_stats(self, policy_id: str,
        ...
        if last_n is None:
            if _adma_settings is not None:
                last_n = _adma_settings.reward_log.stats_last_n
            else:
                last_n = 1000
        rows = self._conn.execute(
    ...
    def close(self) -> None:
        """Close the database connection.  Thread-safe and idempotent.  (A03)"""
        with self._lock:
            if self._closed:
                return
            self._closed = True
