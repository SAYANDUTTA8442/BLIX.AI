...
def _default_reward_cfg():
    """Return reward_engine config from the module-level singleton (A19: fail-fast import)."""
    if _adma_settings is None:
        return None
    return _adma_settings.reward_engine
...
class SystemRewardEngine:
    ...
    def latency_reward(
        ...
        _cfg = _default_reward_cfg()
        _target = target_ms if target_ms is not None else (_cfg.latency_target_ms if _cfg else 500.0)
        value = _normalise_latency(latency_ms, _target)
        return RewardSignal(
            reward_type=RewardType.LATENCY,
            value=value,
            context={"operation": operation, "latency_ms": latency_ms, "target_ms": _target},
            policy_id=policy_id,
