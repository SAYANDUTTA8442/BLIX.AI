...
class AblationV3Report:
    ...
    def format_report(self) -> str:
        """
        Return the ablation report as a formatted string (A16).

        The caller decides what to do with it::

            # log it
            log.info(report.format_report())
        ...
        lines.append("")
        lines.append("=" * 72)
        lines.append("  BLIX v0.3.16 ADMA — ABLATION STUDY v3 (Dependency Injection)")
        lines.append("=" * 72)
        lines.append(
        ...
        lines.append("")
        lines.append(f"{'Condition':<32} {'Score':>7} {'ΔScore':>8} {'Pass':>6} {'Impact':<10}")
        lines.append("─" * 65)
        for row in self.summary_table():
            lines.append(
                f"{row['condition']:<32} {row['overall_score']:>6.4f}  "
    ...
    def print_report(self) -> None:
        """
        Print the ablation report to stdout.

        Deprecated in favour of :meth:`format_report` (A16).
        Use ``log.info(report.format_report())`` in production code.
        """
        log.warning(
            "AblationV3Report.print_report() writes to stdout — "
            "use format_report() and log the result in production code"
        )
