...
def _default_optimizer_cfg():
    """Return optimizer config from the module-level singleton (A19: fail-fast import)."""
    if _adma_settings is None:
        return None
    return _adma_settings.optimizer
...
class PolicyOptimizer:
    ...
    def __init__(
        ...
        _cfg = _default_optimizer_cfg()
        self._store          = policy_store
        self._decay          = decay_factor         if decay_factor         is not None else (_cfg.decay_factor         if _cfg else 0.995)
        self._min_obs        = min_observations     if min_observations     is not None else (_cfg.min_observations     if _cfg else 10)
        self._aging_t        = aging_threshold      if aging_threshold      is not None else (_cfg.aging_threshold      if _cfg else 0.35)
        self._conv_window    = convergence_window   if convergence_window   is not None else (_cfg.convergence_window   if _cfg else 5)
        self._conv_tol       = convergence_tolerance if convergence_tolerance is not None else (_cfg.convergence_tolerance if _cfg else 0.02)
        self._mutation_scale = mutation_scale       if mutation_scale       is not None else (_cfg.mutation_scale       if _cfg else 0.1)
