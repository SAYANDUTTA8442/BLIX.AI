...
class OllamaProvider(LLMProvider):
    ...
    def reset(self) -> None:
        """
        Reset provider state between benchmark queries (A31).

        Ollama's Python client sends a fresh stateless request on every
        ``ollama.chat()`` call (single-turn ``messages`` list), so no
        KV-cache or conversation history accumulates on the Python side.
        This method is a documented no-op for ``OllamaProvider``.

        The Ollama server maintains its own per-session KV cache, but
        that is scoped to the connection and not accessible here.  For
        true server-side isolation, restart Ollama between runs.
        """
        log.debug("OllamaProvider.reset() called — stateless client, no-op")
