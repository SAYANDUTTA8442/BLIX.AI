...
class LLMProvider(ABC):
    ...
    def reset(self) -> None:
        """
        Reset any stateful caches held by this provider (A31).

        Call this between unrelated queries in benchmark / evaluation
        harnesses to ensure isolation:

        - Prevents KV-cache accumulation across queries in HuggingFace
          pipeline-based providers.
        - Clears conversation history if the provider tracks it.
        - Is a safe no-op for stateless providers (e.g. Ollama).

        The default implementation is a no-op so existing subclasses
        that do not override it remain backward-compatible.

        Example usage in an evaluation harness::

            for sample in dataset:
                llm.reset()          # isolate each query
                answer = llm.generate(sample["question"])
        """
        # Default: no persistent state to clear.
