...
class TransformersProvider(LLMProvider):
    ...
    def reset(self) -> None:
        """
        Clear any cross-call state (A31 — KV cache isolation).

        ``transformers.pipeline`` does not accumulate ``past_key_values``
        across independent ``__call__`` invocations, so there is no
        KV-cache state to clear at the Python level.  This method exists
        so benchmark harnesses can call ``llm.reset()`` uniformly without
        special-casing the provider.

        If you are calling ``model.generate()`` directly with stored
        ``past_key_values``, set them to ``None`` here.
        """
        # Pipeline is stateless across calls — nothing to reset.
        # Subclasses that store past_key_values directly should override.
        log.debug("TransformersProvider.reset() called — pipeline is stateless, no-op")
