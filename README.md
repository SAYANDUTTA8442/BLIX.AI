...
[![Version](https://img.shields.io/badge/Version-v0.3.17.8-a5b4fc?style=flat-square)]()
[![Tests](https://img.shields.io/badge/Tests-2%2C625%20passing-22c55e?style=flat-square)]()
[![Audit](https://img.shields.io/badge/Audit%20Backlog-11%20open%20%C2%B7%2020%20resolved-f59e0b?style=flat-square)]()
...
| Silent regressions | Ship and hope | Every known defect is logged, numbered, and closed with a dedicated test before the next version ships — see [Engineering Practice](#-engineering-practice--audit-driven-development) |
...
Blix has grown, over two dozen versioned releases (v0.1 → v0.3.17.8), from a memory-augmented chat tutor into a full cognitive stack of **~30 cooperating subsystems** spanning **~75,000 lines of Python**, each independently testable and injectable for ablation studies.
...
- **Consolidation** — a background engine merges, dedupes, and rolls up memory nodes on a schedule (`consolidation_engine.py`); the original full-table-scan cost on every run (audit item A05) was replaced with an incremental scan in v0.3.17.
...
Static configuration is replaced by **online policy learning** — Blix tunes its own retrieval/planning/prompting behavior from observed outcomes, without gradient updates or RLHF. Introduced in v0.3.16 and hardened significantly through the v0.3.17 audit cycle.
...
- **Cache correctness** — the LRU policy cache no longer double-applies decay on eviction/re-fetch (audit item A01, fixed v0.3.17.0); `PolicyLearner.register()` and the in-memory cache are now thread-safe (A14, A29 — v0.3.17.6).
...
**Full repository test suite: 2,625 tests, all passing** (verified against the v0.3.17.8 source tree). StateBench results specific to the v0.3.1 milestone are documented separately in the 41-page Minor Project I academic report.

---

## 🩺 Engineering Practice — Audit-Driven Development

Past v0.3.16, Blix stopped being fixed reactively and started being audited on a schedule. Every release from **v0.3.17.0 onward** closes a specific, pre-documented batch of issues from a single static + dynamic audit pass across every module — full static analysis, algorithm-correctness review against the guarantees each subsystem is supposed to provide, and comparison against production standards.

Every issue found is written down **before** it's touched: severity, exact subsystem, root-cause mechanism, a concrete failure scenario, a recommended fix, and an effort estimate. Each fix batch ships with its own dedicated test file, so a regression later has somewhere obvious to be caught — `tests/test_v03170_a01_a04.py` through `tests/test_v03178_a18_routed_context_split.py`.

| Batch | Version | Issues closed |
|---|---|---|
| A01–A04 | v0.3.17.0 | LRU double-decay, config error swallowing, HGSHM double-close, reward-log counter reset |
| A05, A06, A11 | v0.3.17.1 | Consolidation full-table scan, hardcoded DB filenames, ignored `mutation_scale` |
| A07–A10 | v0.3.17.2 | Unconditional bandit reward broadcast, `PolicyVersion` field fragility, unsafe `**kwargs`, unbounded shim registry |
| A15 | v0.3.17.3 | Test coverage for `AdaptiveRetriever` / `AdaptivePlanner` |
| A19 | v0.3.17.4 | Deterministic settings-import timing |
| A31 | v0.3.17.5 | KV-cache reset correctness |
| A12–A14 | v0.3.17.6 | Broad exception handling in the retrieval hot path, unsurfaced vector-store fallback, `PolicyLearner` cache thread-safety |
| A16, A17 | v0.3.17.7 | Removed runtime `print()` calls from production modules, added `PolicyVersion` test coverage |
| A18 | v0.3.17.8 | `RoutedContext.to_memory_context()` no longer drops supporting memories |

**Currently open** — 11 issues (A20–A30) are logged and scheduled but not yet closed, including incomplete injection-pattern coverage in `sanitize_task_text` (A20), a silent `INSERT OR IGNORE` on `policy_versions` that can drop data (A26), and a missing `close()` on `EmbeddingManager` (A24). An honestly-documented, numbered backlog is treated as a sign of process health here, not a sign the project is unfinished — there is always a next batch.
...
| API | FastAPI + Uvicorn + python-multipart | REST surface across 20 routers |
| Fuzzy Matching | RapidFuzz | Entity/label deduplication in the memory graph |
| Testing | Pytest | 2,625 tests across every subsystem |
...
- ✅ Zero `TODO`/`FIXME` markers in shipped source — outstanding work lives in the numbered audit backlog, not in code comments
...
├── Issues.md                  # Numbered audit backlog (severity, root cause, fix)
├── CHANGELOG_v0.3.*.md        # Versioned changelogs, v0.3.1 → v0.3.17.8
│
...
└── tests/                       # 2,625 tests across every subsystem
```
...
pip install -e ".[api]"            # FastAPI server — needs fastapi,
                                    #   uvicorn, python-multipart
pip install -e ".[documents]"      # PDF/DOCX ingestion
pip install -e ".[media]"          # Image OCR
```

> **Note:** the API layer (`api/`) requires `fastapi`, `uvicorn`, and `python-multipart` even for local development — install these explicitly if you plan to import anything under `api/` directly rather than through the `[api]` extra.
...
- [x] StateBench benchmark suite — 2,625 passing tests
- [x] First full codebase audit — 30 issues catalogued (`Issues.md`)
- [x] Audit batches A01–A19 and A31 closed across v0.3.17.0–v0.3.17.8

### v0.3.18+ — closing the audit backlog (in progress)
- [ ] A20 — complete injection-pattern coverage in `sanitize_task_text`
- [ ] A21, A26 — schema-migration coverage and `INSERT OR IGNORE` data-loss fix in `policy/store.py`
- [ ] A22–A25, A27–A30 — remaining medium/low-severity items (see [Engineering Practice](#-engineering-practice--audit-driven-development))
- [ ] Portable test paths — remove the last hardcoded absolute paths from `tests/test_v03177_a16_a17.py`
- [ ] Sync `requirements.txt` and `README.md` version banners on every release going forward
...
Blix began as an independent project in **June 2024** — a year before the author entered IIT Patna, and originally born out of a hackathon question at Aignite 2025 about why an AI companion's conversations weren't really personalized. It is the longest-running and most architecturally ambitious project in this portfolio, now on its 24th versioned release.

A **323-page architecture document** covers the full design rationale, subsystem specifications, and theoretical grounding for every component. A **41-page Minor Project I academic report** (Blix v0.3.1) documents the original 329 StateBench tests and their results; the subsequent v0.3.2–v0.3.17.8 changelogs extend this with causal reasoning, metacognition, curiosity-driven experimentation, the ADMA policy-learning layer, and — from v0.3.17 onward — a formal audit-driven hardening process.
...
**Good first issues:** anything in the open A20–A30 audit backlog (see `Issues.md`), documentation improvements, new StateBench test cases, alternative embedding model integrations, additional Ollama model configurations, new policy arms for the ADMA bandit layer.
